...
@section('content')
    <main id="main" class="main pt-0">
        <div class="card">
            ...
            <form method="post" action="{{ route('update.right', $right->id) }}" enctype="multipart/form-data"
                class="row g-3 p-3">
                @csrf
                @method('PUT')
                ...
                <div class="col-md-6 pb-3">
                    ...
                    <input type="text" class="form-control" id="module" name="module" value="{{ $right->module }}"
                        required>
                    @error('module')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
